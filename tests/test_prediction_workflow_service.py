...
class TestPredictionWorkflowService:
    @patch("services.prediction_service.PredictionCacheRepository")
    ...
    async def test_async_predict_success(
        self, mock_item_repo, mock_mod_repo, mock_cache_repo
    ):
        ...
        kafka = AsyncMock()
        service = PredictionWorkflowService(pool=AsyncMock(), redis=AsyncMock())
    ...
    async def test_async_predict_item_not_found(
        ...
        item_repo = mock_item_repo.return_value
        item_repo.get_item_with_user = AsyncMock(return_value=None)
    ...
    async def test_get_moderation_result_not_found(
        ...
        mod_repo = mock_mod_repo.return_value
        mod_repo.get_task = AsyncMock(return_value=None)
    ...
    async def test_close_item_orchestrates_repositories(
        ...
        item_repo = mock_item_repo.return_value
        mod_repo = mock_mod_repo.return_value
        cache_repo = mock_cache_repo.return_value

        item_repo.close_item = AsyncMock(return_value=True)
        ...
        cache_repo.delete_prediction.assert_awaited_once_with(100)
    ...
    async def test_close_item_not_found(
        ...
        item_repo.close_item = AsyncMock(return_value=False)
        mod_repo.delete_by_item_id = AsyncMock(return_value=True)
        cache_repo.delete_prediction = AsyncMock(return_value=None)
        ...
        cache_repo.delete_prediction.assert_not_awaited()
    ...
    async def test_get_moderation_result_completed_caches(
        ...
        cache_repo.set_prediction.assert_awaited_once()
